module sui_bank::bank {
  ...
  fun init(ctx: &mut TxContext) {
    ...
    transfer::share_object(FeePolicy { id: object::new(ctx), fee: 5, ltv: 40 });
  }  
  ...
  public fun deposit(self: &mut Bank, fee_policy: &FeePolicy, account: &mut Account, token: Coin<SUI>, ctx: &mut TxContext) {
    let value = coin::value(&token);
    let deposit_value = value - (((value as u128) * fee_policy.fee / 100) as u64);
  ...
  public fun fee(fee_policy: &FeePolicy): u128 {
    fee_policy.fee
  }

  public fun ltv(fee_policy: &FeePolicy): u128 {
    fee_policy.ltv
  }
  ...
  public fun set_fee(_: &OwnerCap, fee_policy: &mut FeePolicy, fee: u128) {
    fee_policy.fee = fee;
  }

  public fun set_ltv(_: &OwnerCap, fee_policy: &mut FeePolicy, ltv: u128) {
    fee_policy.ltv = ltv;
  }
