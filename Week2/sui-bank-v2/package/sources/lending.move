module sui_bank::lending {
  ...
  use sui_bank::bank::{Self, Account, FeePolicy};
  ...
  public fun borrow(account: &mut Account, cap: &mut CapWrapper, fee_policy: &FeePolicy, price: Price, value: u64, ctx: &mut TxContext): Coin<SUI_DOLLAR> {
    let (latest_result, scaling_factor, _) = oracle::destroy(price);

    let max_borrow_amount = (((((bank::account_balance(account) as u128) * latest_result / scaling_factor) * bank::ltv(fee_policy)) / 100) as u64);
