...
module bank::exercise_tests { 
  ...
  fun test_deposit() {
    ...
    next_tx(scenario_mut, ALICE); 
    {
      ...
      assert_eq(bank::admin_balance(&bank), 7);
      assert_eq(bank::user_balance(&bank, ALICE), 143);
  ...
  fun test_withdraw() {
    ...
    next_tx(scenario_mut, ALICE); 
    {
      ...
      let coin_out = bank::withdraw(&mut bank, ctx(scenario_mut));
  ...
  fun test_partial_withdraw() {
    ...
    next_tx(scenario_mut, ALICE); 
    {
      ...
      let coin_out = bank::partial_withdraw(&mut bank, 10, ctx(scenario_mut));

      assert_eq(burn_for_testing(coin_out), 10);
      assert_eq(bank::user_balance(&bank, ALICE), 85);
  ...
  fun test_claim() {
    ...
    next_tx(scenario_mut, ADMIN); 
    {
      ...
      let coin_out = bank::claim(&cap,&mut bank, ctx(scenario_mut));
  ...
  fun test_partial_withdraw_not_enough_balance() {
    ...
    next_tx(scenario_mut, ALICE); 
    {
      ...
      let coin_out = bank::partial_withdraw(&mut bank, 1001, ctx(scenario_mut));
  ...
  fun init_bank(): Scenario {
   let scenario = test_scenario::begin(ADMIN);
    ...
    next_tx(scenario_mut, ADMIN);
    {
      bank::init_for_testing(ctx(scenario_mut));
    };
  ...
  fun deposit(bank: &mut Bank, value: u64, scenario_mut: &mut Scenario) {
    bank::deposit(bank, mint_for_testing(value, ctx(scenario_mut)), ctx(scenario_mut));
  }
}